...
{
    [ApiController] // this is how we configure the class as an api controller, similar to how Spring works, this sets the attribute
    [Route("[controller]")]
    public class ItemController : ControllerBase // this is how inheritance is done, ":" denotes the ihertiance from another class
    {
        private static readonly IEnumerable<ItemModel> Items = new[] // static items to use for now, integrate SQL server later
        {
        ...
        [HttpGet("{itemType:int}")] // make sure this is an http get request, and that type int is passed when this is called
        public ItemModel[] Get(int itemType)
        {
            ItemModel[] items = Items.Where(i => i.ItemType == itemType).ToArray();
            System.Threading.Thread.Sleep(2000); // artifical delay while data loads
